...
class SARSAX:
    ...
    def update(
        ...
        expected_q = np.dot(
            self.q_table[next_state], self.get_action_probabilities(next_state))
    ...
    def get_action_probabilities(self, state):
        max_action = self.get_action(state, "epsilon-greedy")
        prob = self.epsilon / self.actions_n * np.ones(self.actions_n)
        prob[max_action] += 1 - self.epsilon
        return prob
