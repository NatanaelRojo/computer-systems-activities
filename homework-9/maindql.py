...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots()
    ax.plot(rewards, label='Recompensa acumulada DoubleQLearning')
    ax.plot(np.arange(window_size-1, len(rewards)),
            rolling_mean, label='Promedio DoubleQLearning')
    ax.set_xlabel('Episodios')
    ax.set_ylabel('Recompensa')
    ax.set_title(
        ' DoubleQLearning en ' + ENVIRONMENT)
    ax.legend()
    plt.savefig('DoubleQLearning.png',
